...
html_invitation = "\
        ...
        <table border='0' cellspacing='10' cellpadding='0' width='100%%'\
            style='font-family: Verdana, Sans-serif; font-size: 14'>\
            <tr><td width='100%%'>Hi,</td></tr>\
            <tr><td width='100%%'>You are invited to a meeting.</td></tr>\
        </table>\
        <table cellspacing='0' cellpadding='5' border='0' summary='' style='width: 430px; font-family: Arial, Sans-serif; border: 1px Solid #ccc; background-color: #f6f6f6'>\
            <tr valign='center' align='center' style='color: #ffffff;'>\
                <td style='background: #77bc1f; height: 30px; font-size: medium;'>\
                    <div style='font-weight: bold; font-size: medium;'>%(name)s</div>\
                </td>\
            </tr>\
            <tr>\
                <td>\
                <table cellpadding='8' cellspacing='0' border='0' style='font-size: 13' summary='Eventdetails' bgcolor='f6f6f6' width='100%%'>\
                    <tr>\
                        <td width='25%%'><div><b>Start date : </b></div></td>\
                        <td>%(start_date)s</td>\
                    </tr>\
                    <tr>\
                        <td width='15%%'><div><b>Stop date : </b></div></td>\
                        <td width='50%%'>%(end_date)s</td>\
                    </tr>\
                    <tr>\
                        <td width='15%%'><div><b>Duration : </b></div></td>\
                        <td width='50%%'>%(duration)s</td>\
                    </tr>\
                    <tr valign='top'><td><b>Description : </b></td><td>%(description)s</td>\
                    </tr>\
                    <tr valign='top'>\
                        <td><div><b>Place : </b></div></td>\
                        <td>%(location)s</td>\
                    </tr>\
                    <tr valign='top'>\
                        <td><div><b>Attendees : </b></div></td>\
                        <td><div><div>%(attendees)s</div></div></td>\
                    </tr>\
                </table>\
                </td>\
            </tr>\
        ...
        <table border='0' cellspacing='10' cellpadding='0' width='100%%'\
            ...
            <tr><td><a href=%(url_ac)s>Accept...</a></td></tr>\
            <tr><td><a href=%(url_te)s>Tentative...</a></td></tr>\
            <tr><td><a href=%(url_de)s>Reject...</a></td></tr>\
            <tr><td><a href=%(new_term)s>Propose a new date</a></td></tr>\
            <tr></br><td width='100%%'>%(user)s</td></tr>\
            <tr valign='top'><td width='100%%'>-<font color='a7a7a7'>-------------------------</font></td></tr>\
            <tr><td width='100%%'> <font color='a7a7a7'>%(sign)s</font></td></tr>\
        </table>\
        </body>\
        </html>\
        "
...
property or property parameter."),
        ...
        'meeting_ids': fields.many2many('crm.meeting', 'meeting_attendee_rel',\
                            'attendee_id', 'event_id', 'meetings', states={'done': [('readonly', True)]}),
    }
    _defaults = {
        'state': 'needs-action',
    ...
    def _send_mail(self, cr, uid, ids, mail_to, email_from=tools.config.get('email_from', False), context=None):
        ...
        for att in self.browse(cr, uid, ids, context=context):
            ...
            if res_obj:
                ...
                att_infos = []
                sub_str = 'Meeting invitation: '
                if att.partner_id.lang == 'pl_PL':
                    transl = translation_obj.search(cr, uid, [('src','=',sub_str)])
                    if transl:
                        trans = translation_obj.browse(cr, uid, transl)[0]
                        sub_str = trans.value
                sub = sub_str+res_obj.name
                ...
                url = ("http://%s/?db=%s#id=%s&view_type=form&model=crm.meeting")%(jp_crm, cr.dbname, res_obj.id)
                url_ac = "http://%s/wp-content/plugins/jobsplus-integration/api/accept_meeting.php?attendee_id=%s&status=accepted"%(jp_www, att.id)
                url_de = "http://%s/wp-content/plugins/jobsplus-integration/api/accept_meeting.php?attendee_id=%s&status=declined"%(jp_www, att.id)
                url_te = "http://%s/wp-content/plugins/jobsplus-integration/api/accept_meeting.php?attendee_id=%s&status=tentative"%(jp_www, att.id)
            
                body_vals = {
                            'name': res_obj.name,
                            'start_date': date_start.strftime("%Y-%m-%d %H:%M:%S"),
                            'end_date': date_stop and date_stop.strftime("%Y-%m-%d %H:%M:%S") or False,
                ...
                invitation = html_invitation
                if att.partner_id.lang == 'pl_PL':
                    transl = translation_obj.search(cr, uid, [('src','=',invitation)])
                    if transl:
                        trans = translation_obj.browse(cr, uid, transl)[0]
                        invitation = trans.value
                body = invitation % body_vals
    ...
    def _make_url(self, cr, uid, res_id, model, context=None):
        ...
        if base_url:
            base_url += '/?db=%s#id=%s&view_type=form&model=%s' % (cr.dbname, res_id, model)
        return base_url
    ...
    def do_tentative(self, cr, uid, ids, context=None, *args):
        ...
        attendee_ids = self.browse(cr, uid, self.search(cr, uid, [('id','=',ids[0])]))[0].meeting_ids[0].attendee_ids
        for attendee in attendee_ids:
            if attendee.id == ids[0]:
                user_name = attendee.partner_id.name
                break
           
        meeting_id = self.browse(cr, uid, self.search(cr, uid, [('id','=',ids[0])]))[0].meeting_ids[0].id
        
        body = _('<b>Attendee: </b>%s<li><b>Change state: </b>tentative</li>')%(user_name)
        
        self.pool.get('crm.meeting').message_post(cr, uid, meeting_id, body=body, type='email', subtype='mail.mt_comment', context=context)
...
#                mod_obj.copy(cr, uid, vals.ref.id, default=defaults, context=context)
        ...
        body = _('<b>Attendee: </b>%s<li><b>Change state: </b>accepted</li>')%(user_name)
    ...
    def do_decline(self, cr, uid, ids, context=None, *args):
        ...
        body = _('<b>Attendee: </b>%s<li><b>Change state: </b>declined</li>')%(user_name)
    ...
    def do_run_scheduler(self, cr, uid, automatic=False, use_new_cursor=False, \
        ...
        for alarm in self.browse(cr, uid, alarm_ids, context=context):
            ...
            if hasattr(res_obj, 'rrule') and res_obj.rrule:
                ...
                trigger_date = datetime.strptime(alarm.trigger_date, '%Y-%m-%d %H:%M:%S')
                for rdate in recurrent_dates:
                    rdate = rdate.strftime('%Y-%m-%d %H:%M:%S')
                    rdate= datetime.strptime(rdate, '%Y-%m-%d %H:%M:%S')
                    if rdate + delta > current_datetime:
                        break
                    if rdate + delta <= current_datetime:
                        if trigger_date <= current_datetime:
            ...
            if re_dates:
                if alarm.action == 'email':
                    datetime_trigger = alarm.event_date
                    datetime_trigger = datetime.strptime(datetime_trigger, '%Y-%m-%d %H:%M:%S')
                    datetime_trigger = fields.datetime.context_timestamp(cr, uid, datetime_trigger, context=context)
                    datetime_trigger = datetime_trigger.strftime('%Y-%m-%d %H:%M')
                    
                    jp_config_obj = self.pool.get('jp.config.settings')
                    jp_config_id = jp_config_obj.search(cr, uid, [])[-1]
                    jp_crm = jp_config_obj.browse(cr, uid, jp_config_id).jobsplus_crm
                    url = ("http://%s/?db=%s#id=%s&view_type=form&model=crm.meeting")%(jp_crm, cr.dbname, alarm.res_id)
                      
                    translation_obj = self.pool.get('ir.translation')
                    
                    if hasattr(res_obj, 'attendee_ids'):
                        for att in res_obj.attendee_ids:
                            sub_str = 'Event reminder: %s'
                            body = """<pre style="font-size: 12px;">
This is an event reminder for: %s<br/>
Event date: %s<br/>
Event description: %s<br/>

From:<br/>
      %s<br/>
<a href='%s'>Link to the event</a><br/>
----<br/>
%s
</pre>
"""
                            mail_to = att.user_id.email
                            if att.partner_id.lang == 'pl_PL':
                                transl = translation_obj.search(cr, uid, [('src','like',body)])
                                transl_sub = translation_obj.search(cr, uid, [('src','=',sub_str)])
                                if transl:
                                    trans = translation_obj.browse(cr, uid, transl)[0]
                                    body = trans.value
                                if transl_sub:
                                    trans_sub = translation_obj.browse(cr, uid, transl_sub)[0]
                                    sub_str = trans_sub.value
                            
                            if alarm.description == False:
                                descr = ''
                            else:
                                descr = alarm.description
                                
                            vals = {}
                            if mail_to:
                                vals = {
                                    'state': 'outgoing',
                                    'subject': sub_str % (alarm.name),
                                    'body_html': body % (alarm.name, datetime_trigger, descr, \
                                                        alarm.user_id.name, url, alarm.user_id.signature),
                                    'email_to': mail_to,
                                    'email_from': 'crm@jobsplus.pl',
                                }
                                self.pool.get('mail.mail').create(cr, uid, vals, context=context)
