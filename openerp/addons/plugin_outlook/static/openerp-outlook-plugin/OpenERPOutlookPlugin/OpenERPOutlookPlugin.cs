...
namespace OpenERPOutlookPlugin
{
    ...
    public class OpenERPOutlookPlugin : Model
    {
        ...
        public Boolean PushMail(outlook.MailItem mail, string model, int thread_id, bool open=true)
        {
            /*
             ...
             */
                ...
                args.Add(String.IsNullOrEmpty(mail.Body)?"":mail.Body);
                args.Add(String.IsNullOrEmpty(mail.HTMLBody) ? "" : mail.HTMLBody);
        ...
        public void CreateCandidateRecord(outlook.MailItem item, long deal_id, string name, string phone, string email_id)
        {
            /*
            ...
            */
            Hashtable values = new Hashtable();
            values.Add("candidate", name);
            values.Add("phone", phone);
            values.Add("email", email_id);
            ArrayList args = new ArrayList();
            args.Add(values);
            args.Add(deal_id);
            object[] contact = (object[])this.openerp_connect.Execute("plugin.handler", "candidate_create", args.ToArray());
            PushMail(item, contact[0].ToString(), Int32.Parse(contact[1].ToString()));

            //this.RedirectWeb(contact[2].ToString());

        }

        public void CreateCandidateRecord(outlook.MailItem item, long deal_id, string name, string phone, string email_id, string position, bool open=true)
        {
            /*
            ...
            */
            ...
            values.Add("phone", phone);
            values.Add("email", email_id);
            values.Add("current_possition", position);
            ArrayList args = new ArrayList();
            args.Add(values);
            args.Add(deal_id);
            object[] contact = (object[])this.openerp_connect.Execute("plugin.handler", "candidate_create", args.ToArray());
            PushMail(item, contact[0].ToString(), Int32.Parse(contact[1].ToString()), open);

            //this.RedirectWeb(contact[2].ToString());

        }

        public Record[] GetDealList()
        {
            /*
             * Gives name and model for search record.
             */
            ArrayList object_list = new ArrayList();
            ArrayList args = new ArrayList();
            Model parent_model = new Model("jp.deal");

            object[] objects = (object[])this.openerp_connect.Execute("plugin.handler", "list_deal_get", args.ToArray());

            object_list.Add(new Record(0, "", parent_model));
