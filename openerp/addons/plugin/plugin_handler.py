...
class plugin_handler(osv.osv_memory):
    ...
    def push_message(self, cr, uid, model, email, res_id=0, body=False):
        """
            @param email: email is a standard RFC2822 email message
    ...
    def list_deal_get(self, cr, uid):
        """
            ...
            [(id, 'name')]
        """
        return self.pool.get('jp.deal').list_deal_get(cr, uid)
        
    def candidate_create(self, cr, uid, data, deal_id, context=None):
        """
            @param data : the data use to create the jp.candidate
                [('field_name', value)], field name and phone is required
            @param deal_id : On which deal the candidate is attached
             if partner_id = 0 then create a new partner with the same name that the address
            @return : the candidate_id created, this allow the plugin to open the right candidate page
        """
        dictcreate = dict(data)
        
        candidate_id = self.pool.get('jp.candidate').create(cr, uid, dictcreate)        
        ...
        if deal_id and deal_id != 0:
            ...
            self.pool.get('jp.application').create(cr,uid, application_vals)
            
        
        url = self._make_url(cr, uid, candidate_id, 'jp.candidate')
        return ('jp.candidate', candidate_id, url)
